...
1. `P1` Normalize heavy-tool guardrails with one common contract (size/type/recovery hints).
2. `P1` Refactor `lib/tools-registry.ts` into modular category files with aggregator compatibility.
3. `P2` Create tool landing content template and link graph enforcement checks.
4. `P2` Add benchmark harness for PDF/Finance core quality gates.
